...
def load_samples(datasets, parameters):
    samp_infos = {}
    samp_info_total = SamplesInfo(year=parameters['year'], out_path=parameters['out_path'],\
                            server=parameters['server'], datasets_from='purdue', debug=False)
    for d in datasets:
        if d in samp_info_total.samples: continue
        si = load_sample(d,parameters)[d]
        if 'files' not in si.fileset[d].keys(): continue
...
def submit_job(arg_set, parameters):
    ...
    executor_args={'nano': True, 'client': parameters['client'], 'compression':None}
...
#            if 'dy_m105_160_amc' not in sample: continue
    ...
    if parameters['local_cluster']:
        parameters['client'] = dask.distributed.Client(processes=True, n_workers=40,\
