...
extern "C"
{
  RoccoR* RoccoR_new(char * filename)
  {
    return new RoccoR(filename);
  }
  
  double RoccoR_kScaleDT(RoccoR* roccor,
			 int Q, double pt, double eta,
			 double phi, int s=0, int m=0)
  {
    return roccor->kScaleDT(Q, pt, eta, phi, s, m);
  }

  double RoccoR_kSpreadMC(RoccoR* roccor,
			  int Q, double pt, double eta, double phi,
			  double gt, int s=0, int m=0)
  {
   return roccor->kSpreadMC(Q, pt, eta, phi, gt, s, m);
  }

  double RoccoR_kSmearMC(RoccoR* roccor,
			  int Q, double pt, double eta, double phi,
			  int n, double u, int s=0, int m=0)
  {
    return roccor->kSmearMC(Q, pt, eta, phi, n, u, s, m);
  }
