...
def cs_variables_old(mu1,mu2,two_muons):
    dphi = abs(np.mod(mu1.phi[two_muons] - mu2.phi[two_muons] + np.pi, 2*np.pi) - np.pi)
...
#https://root.cern.ch/doc/master/classTVector3.html#a5fcc2bc19cf8c84215eb8e50cedae08f
def angle(vec1,vec2):
...
#https://root.cern.ch/doc/master/classTVector3.html#a4d0080544bc4d4ac669fa5e3bada8e25
def unit(vec):
...
#https://root.cern.ch/doc/master/classTVector3.html#ad4464ec846c85cadce7afe9c09c7e245
def cross(vec1,vec2): 
    return [vec1[1]*vec2[2]-vec2[1]*vec1[2],vec1[2]*vec2[0]-vec2[2]*vec1[0],vec1[0]*vec2[1]-vec2[0]*vec1[1]]

#https://root.cern.ch/doc/master/classTLorentzVector.html#a8d77f01dc7f409b237937012c382fbfb
...
#https://root.cern.ch/doc/master/classTRotation.html#acf7e3da35816052c3b6e73f7fe71239d
def rotate_axes(newX,newY,newZ):
    return([ [newX[0],newY[0],newZ[0]], [newX[1],newY[1],newZ[1]], [newX[2],newY[2],newZ[2]] ])

#https://root.cern.ch/doc/master/classTRotation.html#afe12d186ccecf5d71adea56765021f5c
def multiply_mtx(mtx1,mtx2):
    return [ [mtx1[0][0]*mtx2[0][0]+mtx1[0][1]*mtx2[1][0]+mtx1[0][2]*mtx2[2][0],\
...
# https://root.cern.ch/doc/master/classTRotation.html#a4b2d021944f997bb77a6a29597b2a720
def invert(rot):
...
def cs_variables(mu1,mu2,two_muons):
    ...
    mu1_e  = np.sqrt(mu1_px**2 + mu1_py**2 + mu1_pz**2 + mu1.mass[two_muons]**2).flatten()
    ...
    mu2_e  = np.sqrt(mu2_px**2 + mu2_py**2 + mu2_pz**2 + mu2.mass[two_muons]**2).flatten()
    ...
    pf = (np.full(len(px),0),np.full(len(px),0),np.full(len(px),-6500),np.full(len(px),6500))
    pw = (np.full(len(px),0),np.full(len(px),0),np.full(len(px),6500),np.full(len(px),6500))
    ...
    angle_filter = angle([px,py,pz],[pf[0],pf[1],pf[2]])<angle([px,py,pz],[pw[0],pw[1],pw[2]])
    for i in range(4):
    ...
    theta_cs = np.arctan2(np.sqrt(muminus[0]*muminus[0]+muminus[1]*muminus[1]),muminus[2])
