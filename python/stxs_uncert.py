stxs_acc =  {#STXS   TOT   ,  PTH200,  Mjj60 , Mjj120 , Mjj350 , Mjj700 ,Mjj1000 ,Mjj1500  ,  25       , JET01
...
uncert_deltas = [14.867, 0.394, 9.762, 6.788, 7.276, 3.645, 2.638, 1.005, 20.073, 18.094]
powheg_xsec = {  
   200:  273.952 ,
   201:  291.030 ,
   202: 1317.635 ,
...
def stxs_lookups():
    ...
    powheg_xsec_lookup = dense_lookup.dense_lookup(np.array(list(powheg_xsec.values())), [np.array(list(powheg_xsec.keys()))])
    powheg_xsec_lookup._axes = powheg_xsec_lookup._axes[0]
    return stxs_acc_lookups, powheg_xsec_lookup
        

def vbf_uncert_stage_1_1(source, event_STXS, Nsigma, stxs_acc_lookups, powheg_xsec_lookup):
    #return a single weight for a given souce
    if(source < 10):
        delta_var = stxs_acc_lookups[source](event_STXS) * uncert_deltas[source]
        return  np.ones(len(event_STXS), dtype=float) + Nsigma * (delta_var/powheg_xsec_lookup(event_STXS))
