...
#        for
            ...
            dimuon_variable_names = ['dimuon_mass', 'dimuon_mass_res', 'dimuon_mass_res_rel', 'dimuon_ebe_mass_res',
                                     'dimuon_ebe_mass_res_rel', 'dimuon_pt','dimuon_pt_log', 'dimuon_eta', 'dimuon_phi',
                                     'dimuon_dEta','dimuon_dPhi', 'dimuon_dR', 'dimuon_rap',
                                     'dimuon_cos_theta_cs','dimuon_phi_cs']

            for n in mu1_variable_names+mu2_variable_names+dimuon_variable_names:
                output[n] = 0.0
            ...
            mu_for_trigmatch = muons[(muons.pt_raw > self.parameters["muon_leading_pt"]) #&\
                                  # (df.Muon.pfRelIso04_all < self.parameters["muon_trigmatch_iso"]) &\
            ...
            mm_pt, mm_eta, mm_phi, mm_mass, mm_rap = p4_sum(mu1[two_muons], mu2[two_muons])
            ...
            mm_deta, mm_dphi, mm_dr = delta_r(mu1[two_muons].eta.flatten(),\
            ...
            output['dimuon_ebe_mass_res'][two_muons] = mass_resolution_purdue(is_mc,self.evaluator,mu1,mu2,\
                                                                              output['dimuon_mass'], two_muons, self.year)
            output['dimuon_ebe_mass_res_rel'][two_muons]=output['dimuon_mass_res'][two_muons]/output['dimuon_mass'][two_muons]


            output['dimuon_mass_res_rel'][two_muons] = mass_resolution_pisa(self.extractor, mu1, mu2, two_muons)
            output['dimuon_mass_res'][two_muons] = output['dimuon_mass_res_rel'][two_muons]*output['dimuon_mass'][two_muons]
            
            cos_theta_cs, phi_cs = cs_variables(mu1,mu2,two_muons)
            output['dimuon_cos_theta_cs'][two_muons] = cos_theta_cs.flatten()
            ...
            output['mu1_iso'][two_muons] = mu1[two_muons].pfRelIso04_all.flatten()
            output['mu1_pt_over_mass'][two_muons] = np.divide(mu1[two_muons].pt.flatten(), output['dimuon_mass'][two_muons])
            ...
            output['mu2_iso'][two_muons] = mu2[two_muons].pfRelIso04_all.flatten()
            output['mu2_pt_over_mass'][two_muons] = np.divide(mu2[two_muons].pt.flatten(), output['dimuon_mass'][two_muons])
...
#            pt_gen_jet = df.Jet.matched_genjet.pt.flatten(axis=0)
        ...
        if is_mc:
            ...
            if ('dy' in dataset) and False: # disable for now
                ...
                zpt_weight[two_muons] = self.evaluator[self.zpt_path](output['dimuon_pt'][two_muons]).flatten()
...
#            weights.add_weight_with_variations('muTrig', muTrig, muTrig_up, muTrig_down)
            ...
            if ('nominal' in self.pt_variations):
                try:
                    ...
                    weights.add_only_variations('LHEFac', np.full(output.shape[0], np.nan, dtype='float64'),\
                                                np.full(output.shape[0], np.nan, dtype='float64'))
                    weights.add_only_variations('LHERen', np.full(output.shape[0], np.nan, dtype='float64'),\
                                                np.full(output.shape[0], np.nan, dtype='float64'))
        ...
        output.columns = pd.MultiIndex.from_product([output.columns, ['']], names=['Variable', 'Variation'])
        for v_name in jet_variation_names:
            output = self.jec_loop(v_name, is_mc, df, dataset, mask, muons, mu1, mu2, two_muons, weights, numevents, genJetMass, output)
        if self.timer:
        ...
        output.loc[((mass>110) & (mass<115.03)) | ((mass>135.03) & (mass<150)), 'r'] = "h-sidebands"
        output.loc[((mass>115.03) & (mass<135.03)), 'r'] = "h-peak"
        output['s'] = dataset
        output['year'] = int(self.year)
        
        for wgt in weights.df.columns:
            if ('up' not in wgt) and ('down' not in wgt): continue
            output[f'wgt_{wgt}'] = weights.get_weight(wgt) 
        
        columns_to_save = [c for c in output.columns if (c[0] in self.vars_to_save) or ('wgt_' in c[0]) or\
                           ('mcreplica' in c[0]) or (c[0] in ['c','r','s','year'])]
        output = output[columns_to_save]
        output = output.reindex(sorted(output.columns), axis=1)

        output = output[output.loc[:,pd.IndexSlice['c','nominal']].isin(self.channels)]
    ...
    def jec_loop(self, variation, is_mc, df, dataset, mask, muons, mu1, mu2, two_muons, weights, numevents, genJetMass, output):
        ...
        variable_names = [
            'jet1_pt', 'jet1_eta', 'jet1_rap', 'jet1_phi', 'jet1_qgl','jet1_id', 'jet1_puid', 'jet1_has_matched_muon',
            'jet1_matched_muon_dr', 'jet1_matched_muon_pt', 'jet1_matched_muon_iso', 'jet1_matched_muon_id',
            'jet2_pt', 'jet2_eta', 'jet2_rap', 'jet2_phi', 'jet2_qgl', 'jet2_id', 'jet2_puid', 'jet2_has_matched_muon',
            'jet2_matched_muon_dr', 'jet2_matched_muon_pt', 'jet2_matched_muon_iso', 'jet2_matched_muon_id',
            'jj_mass', 'jj_mass_log', 'jj_pt', 'jj_eta', 'jj_phi', 'jj_dEta', 'jj_dPhi',
            'mmj1_dEta','mmj1_dPhi','mmj1_dR','mmj2_dEta','mmj2_dPhi','mmj2_dR','mmj_min_dEta', 'mmj_min_dPhi',
            'mmjj_pt', 'mmjj_eta', 'mmjj_phi', 'mmjj_mass', 'rpt', 'zeppenfeld','ll_zstar_log',
            'nsoftjets2', 'nsoftjets5', 'htsoft2', 'htsoft5'
        ]
        variables = {}
...
#        deltar_mujet_ok = ((match_mu.pfRelIso04_all>0.25) | (~match_mu.mediumId) | (match_mu.pt<20)).all().flatten()
        ...
        variables['jet1_has_matched_muon'][one_jet] = jet1.has_matched_muon
        variables['jet1_matched_muon_dr'][one_jet] = jet1.matched_muon_dr
        variables['jet1_matched_muon_pt'][one_jet] = jet1.matched_muon_pt
        variables['jet1_matched_muon_iso'][one_jet] = jet1.matched_muon_iso
        variables['jet1_matched_muon_id'][one_jet] = jet1.matched_muon_id
        ...
        variables['jet2_has_matched_muon'][two_jets] = jet2.has_matched_muon
        variables['jet2_matched_muon_dr'][two_jets] = jet2.matched_muon_dr
        variables['jet2_matched_muon_pt'][two_jets] = jet2.matched_muon_pt
        variables['jet2_matched_muon_iso'][two_jets] = jet2.matched_muon_iso
        variables['jet2_matched_muon_id'][two_jets] = jet2.matched_muon_id

        variables['jj_pt'][two_jets],\
        variables['jj_eta'][two_jets],\
        variables['jj_phi'][two_jets],\
        variables['jj_mass'][two_jets],_ = p4_sum(jet1[two_jets[one_jet]], jet2)
        variables['jj_mass_log'] = np.log(variables['jj_mass'])
        
        variables['jj_dEta'][two_jets],\
        variables['jj_dPhi'][two_jets], _ = delta_r(variables['jet1_eta'][two_jets],\
        ...
        variables['zeppenfeld'][two_muons&two_jets] = (output['dimuon_eta'][two_muons&two_jets] - 0.5*\
                          (variables['jet1_eta'][two_muons&two_jets] + variables['jet2_eta'][two_muons&two_jets]))
        
        variables['mmjj_pt'][two_muons&two_jets],\
        variables['mmjj_eta'][two_muons&two_jets],\
        variables['mmjj_phi'][two_muons&two_jets],\
        variables['mmjj_mass'][two_muons&two_jets] = p4_sum_alt(output['dimuon_pt'][two_muons&two_jets],\
        ...
        variables['rpt'][two_muons&two_jets] = variables['mmjj_pt'][two_muons&two_jets] / \
        ...
        ll_ystar[two_muons&two_jets] = output['dimuon_rap'][two_muons&two_jets] -\
                (variables['jet1_rap'][two_muons&two_jets]+variables['jet2_rap'][two_muons&two_jets])/2
        ll_zstar[two_muons&two_jets] = abs(ll_ystar[two_muons&two_jets] / \
                             (variables['jet1_rap'][two_muons&two_jets]-variables['jet2_rap'][two_muons&two_jets]))
        variables['ll_zstar_log'][two_muons&two_jets] = np.log(ll_zstar[two_muons&two_jets])

        
        variables['mmj1_dEta'][two_muons&one_jet],\
        variables['mmj1_dPhi'][two_muons&one_jet],\
        variables['mmj1_dR'][two_muons&one_jet] = delta_r(output['dimuon_eta'][two_muons&one_jet],\
        ...
        variables['mmj2_dR'][two_muons&two_jets] = delta_r(output['dimuon_eta'][two_muons&two_jets],\
                                                       variables['jet2_eta'][two_muons&two_jets].flatten(),\
                                                       output['dimuon_phi'][two_muons&two_jets],\
                                                       variables['jet2_phi'][two_muons&two_jets].flatten())

        variables['mmj_min_dEta'] = np.where(variables['mmj1_dEta'],\
                                                 variables['mmj2_dEta'],\
                                                 (variables['mmj1_dEta'] < variables['mmj2_dEta']))
        
        variables['mmj_min_dPhi'] = np.where(variables['mmj1_dPhi'],\
                                                 variables['mmj2_dPhi'],\
        ...
        variables['htsoft2'] = self.get_softjet_vars(df, df.SoftActivityJet, 2, muons, mu1, mu2,\
                                                     jet1, jet2, two_muons, one_jet, two_jets)
        
        variables['nsoftjets5'],\
        variables['htsoft5'] = self.get_softjet_vars(df, df.SoftActivityJet, 5, muons, mu1, mu2,\
                                                     jet1, jet2, two_muons, one_jet, two_jets)
        ...
        vbf_cut = (variables['jj_mass']>400)&(variables['jj_dEta']>2.5)&leading_jet_pt
        ...
        mass = (output['dimuon_mass']>115) & (output['dimuon_mass']<135)
        ...
        # so even variables that don't directly depend on jet pT (e.g. QGL) 
        # must be saved for all pT variations.
